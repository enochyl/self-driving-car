...
def region_of_interest(img, vertices):
    ...
    INPUT:
                 img : Image
            vertices : Vertices of the Region of interest

    OUTPUT:
        masked_image : Image Image where mask pixels are nonzero
    """
...
def hough_lines_detection(img, rho, theta, threshold, min_line_len, max_line_gap):
    """
    PURPOSE: Detect lines based on Probabilistic Hough Line Transform
    ...
    INPUT:
        ...
        min_line_len : The minimum number of intersections to “detect” a line
        max_line_gap : The maximum gap between two points to be considered in the same line

    OUTPUT:
               lines : extremes of the detected lines
    """
...
def weighted_img(img1, img2, α=0.8, β=1., λ=0.):
    """
    PURPOSE:
        Blend image as follow: img2 * α + img1 * β + λ
    ...
    INPUT:
                ...
                img2 : Second input image array
                   α : Weight
                   β : Weight
                   λ : Scalar added to each sum

    OUTPUT:
        weighted_img : Weighted sum of two image arrays
    """
    img1 = np.uint8(img1)
    if len(img.shape) == 2:
        img1 = np.dstack((img1, np.zeros_like(img1), np.zeros_like(img1)))

    return cv2.addWeighted(img2, α, img1, β, λ)
...
def get_lane_lines(color_image, solid_lines=True):
    ...
    INPUT:
        color_image : input frame
        solid_lines : if True, only selected lane lines are returned. If False, all candidate lines are returned.

    OUTPUT:
         lane_lines : list of (candidate) lane lines.
    """
    # resize to 960 x 540
...
def smoothen_over_time(lane_lines):
    ...
    INPUT:
        lane_lines : Lane line reference

    OUTPUT:
              Line : Updated average lines
    """
...
def color_frame_pipeline(frames, solid_lines=True, temporal_smoothing=True):
    """
    PURPOSE:
        Overlaying the inferred road lines

    INPUT:
           frames : list of RGV frames

    OUTPUT:
        img_blend : Overlaid image with inferred road lanes
                    Takes as input a list of frames (RGB) and returns an image (RGB)
    """
    ...
    vertices = np.array([[(50, img_h),
                          ...
                          (img_w - 50, img_h)]],
                        dtype=np.int32)
    img_masked = region_of_interest(line_img, vertices)
